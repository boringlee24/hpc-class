...
__global__ void stencil(float* a, float* b)
{
    ...
    {
        a[idx+N*(idy+N*idz)] = 0.8 * (b[(idx-1)+N*(idy+N*idz)] + b[(idx+1)+N*(idy+N*idz)] + 
                                b[idx+N*((idy-1)+N*idz)] + b[idx+N*((idy+1)+N*idz)] + 
                                b[idx+N*(idy+N*(idz-1))] + b[idx+N*(idy+N*(idz+1))]);
    }
    else
    {
        a[idx+N*(idy+N*idz)] = 0;
...
int main (int argc, char** argv) {
    ...
    finish = CLOCK();    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) 
