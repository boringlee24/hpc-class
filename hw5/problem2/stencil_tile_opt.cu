...
__global__ void stencil(float* a, float* b)
{
    ...
    __shared__ float temp[BLOCKSIZE][BLOCKSIZE][2*BLOCKSIZE];
    ...
    if ((idx >= 1) && (idx < N-1) && (idy >= 1) && (idy < N-1) && (idz >= 1) && (idz < N-1))
    {
        ...
        float z_part = (threadIdx.z >= 1 ? temp[threadIdx.x][threadIdx.y][threadIdx.z-1] : b[idx+N*(idy+N*(idz-1))]) + 
                        (threadIdx.z < blockDim.z-1 ? temp[threadIdx.x][threadIdx.y][threadIdx.z+1] : b[idx+N*(idy+N*(idz+1))]);                            
...
int main (int argc, char** argv) {
    ...
    int grid_zdim = ceil(N/(float)BLOCKSIZE/2);
    dim3 threadPerBlock(BLOCKSIZE, BLOCKSIZE, BLOCKSIZE*2);
    dim3 blockPerGrid(grid_dim, grid_dim, grid_zdim);
    printf("block size: %d^2x%d, grid size: %d^2x%d\n", BLOCKSIZE, BLOCKSIZE*2, grid_dim, grid_zdim);
